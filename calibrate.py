...
def main(opts):
    ...
    # NOTE: k=2 means the euclidean distance between the two closest matches
    ...
    for idx in range(num_frames):
        ...
        # Run object detection — person class only, CPU, suppress per-frame logs
        results = detector(
            [frame1, frame2],
            device="cpu",
            classes=[0],
        ...
        # YOLOv8 boxes.data → (x1, y1, x2, y2, conf, cls); we only need xyxy
        pred1 = results[0].boxes.data.cpu().numpy()[:, :4]
        pred2 = results[1].boxes.data.cpu().numpy()[:, :4]

        # Project Camera 1 boxes into Camera 2's coordinate space
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--homography-pth",
        type=str,
        help="Path to save the homography as a .npy file (no need to add the .npy extension).",
    )
